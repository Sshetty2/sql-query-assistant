...
class State(TypedDict):
    ...
    filtered_schema: Optional[
        list[dict]
    ]  # Holds filtered schema (top-k relevant tables with ALL columns - for modification options)  # noqa: E501
    truncated_schema: Optional[
        list[dict]
    ]  # Holds truncated schema (filtered tables with ONLY relevant columns - for planner context)  # noqa: E501
    schema_markdown: Optional[str]  # Schema formatted as markdown for LLM readability
    pre_plan_strategy: Optional[
        str
    ]  # Text-based strategic plan generated by pre-planner (initial queries only)
    revised_strategy: Optional[
        str
    ]  # Revised strategy from error/refinement corrections (bypasses pre-planner)
    planner_outputs: list[dict[str, Any]]  # History of planner outputs
    planner_output: Optional[
        dict[str, Any]
    ]  # Current/latest planner output (convenience field)
    ...
    total_records_available: Optional[
        int
    ]  # Total records available before limit applied
    ...
    # Iteration tracking (separate counters per correction type)
    audit_iteration: int  # Current audit iteration (max: 2)
    ...
    executed_plan: Optional[
        dict[str, Any]
    ]  # The plan that generated currently displayed results
    executed_query: Optional[
        str
    ]  # The SQL query that generated currently displayed results
    ...
    modification_options: Optional[
        dict[str, Any]
    ]  # Available modification options for UI
